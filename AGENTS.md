...
- Three pages, one shared shell: `templates/base.html` owns the head, Chart.js CDN, navbar (logo + profile button), and the search box + suggestion dropdown. Pages extend it and fill the `title` / `content` / `scripts` (and optionally `body_attrs`) blocks. Dashboard and Ledger also fill the `bottom_tabs` block (which includes `_bottom_tabs.html`, a shared partial with icon tabs fixed to the viewport bottom). `static/js/common.js` (formatters, `paintChange`, search UI, `setupTimeframeChart`, `REFRESH_MS`) is loaded by base.html BEFORE each page script — page scripts just use the globals.
- Layering: `market_data.py` = data layer (yfinance + caching, knows nothing about Flask); `db.py` = persistence layer (SQLite, knows nothing about Flask/yfinance); `app.py` = routes (decides WHICH symbols); `static/js/main.js` = dashboard rendering, `static/js/ledger.js` = ledger-page rendering (transaction ledger + Closed sales table — moved off the dashboard), `static/js/stock.js` = detail-page rendering.
...
- Timeframe buttons (1D–MAX) and `/api/portfolio/history` both go through `PERIOD_MAP` (`market_data.py`) — the route validates client-supplied period keys against it. Adding a timeframe = edit it there once. Each entry carries explicit flags — `intraday` (True ONLY for 1D: clock-time labels + the route's first-bar ledger branch), `live` (True for 1D and 5D: short 120s TTL), `label` (strftime format: 1D "HH:MM", 5D "YYYY-MM-DD HH:MM" — any day carrying several bars, else "YYYY-MM-DD"). Never sniff interval strings. 5Y (`1wk`) and MAX (`1mo`) serve coarser bars by design; 5D serves 30-minute bars; everything above 5D except 5Y/MAX serves daily bars (a monotonically dense ladder).
...
- History cache is an in-memory dict in `market_data.py` keyed `(symbol, period)` (TTL 600s settled bars / 120s "live" series — 1D and 5D) — same NOT-SQLite call as the quote cache; dies on restart, acceptable. Successes only: a failed fetch is never cached, so the next click retries. Rework trigger shared with the quote cache (`project-brief.md`).
- Volume-leaders cache is a single-entry in-memory dict in `market_data.py` (TTL 300s) holding the whole computed top-10 list (`SECTOR_LEADERS` scan → per-sector winner → sort → cap) behind `GET /api/market/volume-leaders` — same NOT-SQLite call, dies on restart. Successes only: zero usable leaders raises `ValueError` (never cached `[]`), which the route degrades to 200 + `{"leaders": []}`. Rework trigger shared with the quote cache (`project-brief.md`).
...
- Prod = gunicorn `app:app`, 1 worker + 8 threads — deliberate: the in-memory caches (quote, history, names, volume leaders) and SQLite want a single process. Dev keeps `python app.py`.
...
- Ledger table column count (11, incl. the trailing actions column) lives in FOUR places: the `<th>` row (`templates/ledger.html`), `setLedgerMessage`'s `colSpan`, `buildGroupRow`'s cell list (summary rows end with a BLANK actions cell), AND `buildTxRow`'s cell list (`static/js/ledger.js`). Adding/removing a column = edit all four.
- Closed sales table (on `/ledger`): 8 columns declared ONCE via `data-cs-col` attributes in `ledger.html`; `ledger.js` appends cells in that order and `setClosedSalesMessage`'s `colSpan` must match. Data comes from `GET /api/portfolio/realized` — an average-cost replay of the stored ledger (NO quotes, NO live FX in the money math: realized gains are history built from frozen facts and can never go stale; the ONE network touch is `get_name` for the Name column — process-lifetime cache, one `.info` per sold ticker after a restart, retried per poll while Yahoo can't answer). Rows degrade to `null` CAD fields + a `degraded` reason when a position's history contains a NULL-fx or unsupported-currency leg — never a fake 1:1; `total_realized` goes null if ANY row degraded (a partial sum that looks complete is a lie). The fold is short-symmetric (oversell opens a short at the sell price; a covering BUY books its gain into the total — that gain appears in NO row; rows ⊂ total is documented) and its native pool shrinks unconditionally even when the CAD pool is unrated; the flat-wipe is a 1e-9 TOLERANCE, not `== 0` (fractional qtys leave binary residue). Editing old transactions retroactively rewrites realized history (recompute-on-read). `tests/test_realized.py` locks the math.
