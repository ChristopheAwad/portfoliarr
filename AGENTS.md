...
- Closed sales table (on `/ledger`): 8 columns declared ONCE via `data-cs-col` attributes in `ledger.html`; `ledger.js` appends cells in that order and `setClosedSalesMessage`'s `colSpan` must match. Data comes from `GET /api/portfolio/realized` — an average-cost replay of the stored ledger (NO quotes, NO live FX: realized gains are history built from frozen facts, so they can never go stale and the endpoint makes zero network calls). Rows degrade to `null` CAD fields + a `degraded` reason when a position's history contains a NULL-fx or unsupported-currency leg — never a fake 1:1; `total_realized` goes null if ANY row degraded (a partial sum that looks complete is a lie). The fold is short-symmetric (oversell opens a short at the sell price; a covering BUY books its gain into the total — that gain appears in NO row; rows ⊂ total is documented). Editing old transactions retroactively rewrites realized history (recompute-on-read). `tests/test_realized.py` locks the math.
