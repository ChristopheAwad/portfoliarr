...
#   2. get_history + the portfolio route treat "5m" as the ONLY intraday
#      interval, so 1wk/1mo get date labels and daily-shaped ledger math.
#   3. A history cache in market_data.py: {(symbol, period_key): ...},
#      TTL 600s for settled bars (daily/weekly/monthly), 120s for 1D
#      (today's bar keeps moving). Successes only — failures re-fetch.
...
def test_history_is_cached_within_ttl(fake_yf):
    ...
    re-buy the same data from Yahoo three times."""
    ...
    assert first == second == {"2026-08-28": 100.0, "2026-08-31": 110.0}
...
def test_history_refetches_after_ttl_expires(fake_yf, monkeypatch):
    ...
    faked so the test runs in microseconds, not minutes."""
    fake_yf.state["history"] = pd.DataFrame(
        {"Close": [100.0]},
    ...
    market_data.get_history("AAPL", "5D")      # fetch #1, stamped at t=1000
...
def test_failed_history_fetch_is_never_cached(fake_yf, monkeypatch):
    ...
    market_data.get_history("AAPL", "5D")      # and the success IS cached
    assert len(fake_yf.calls) == 2
